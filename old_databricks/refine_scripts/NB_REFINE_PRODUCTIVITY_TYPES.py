...
js_config = functions.func_get_config(
  str_config_path = constants.CONST_CONFIG_PATH)['refined_tables']['productivity_types']

df_raw_gdms_reference_excel = ps.read_excel(js_config['src_path'], 
  sheet_name = 'ProductivityType', 
  header = 0
)

# Drop the extra column
df_raw_gdms_reference_excel = df_raw_gdms_reference_excel.drop(columns = ['Unnamed: 4'])
